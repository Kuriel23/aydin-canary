...
module.exports = {
  name: 'forceban',
  description: 'Force um banimento!',
  permissions: 'BAN_MEMBERS',
  options: [
    {
      name: 'id',
      description: 'Qual id do usuário?',
      type: 3,
      required: true
    },
    {
  ...
  run: async (interaction, client) => {
    const usuario = interaction.options.getString('id')
    const motivo =
      interaction.options.getString('motivo') ||
      'Bom, nenhum motivo foi fornecido!'
    if (usuario === interaction.message.author.id) { return interaction.reply({ content: 'Você não pode se banir!' }) }
    ...
    guild.bans.create(usuario, { reason, days: 1 })
    interaction.reply({
      content:
        'O usuário ' +
        usuario +
