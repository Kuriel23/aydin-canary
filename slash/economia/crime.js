  ...
  run: async (interaction, client) => {
    ...
    if (doc) {
      ...
      if (delayTime - (Date.now() - doc.crimeCooldown) < 0) {
        ...
        if (result === "Droga") {
          let droga = Math.floor(Math.random() * (50000 - 30000)) + 30000;
          doc.dinsujo += droga;
          doc.crimeCooldown = Date.now();
          doc.save();
          let drogaemb = embed(
        ...
        if (result === "Assassino") {
          let ass = Math.floor(Math.random() * (5000 - 2000)) + 2000;
          doc.animecoins += ass;
          doc.crimeCooldown = Date.now();
          doc.save();
          let assassino = embed(
        ...
        if (result === "Violação") {
          let violar = Math.floor(Math.random() * (5000 - 3000)) + 3000;
          doc.dinsujo += violar;
          doc.crimeCooldown = Date.now();
          doc.save();
          let violou = embed(
        ...
        if (result === "Sequestro") {
          let sequestro = Math.floor(Math.random() * (15000 - 10000)) + 10000;
          doc.dinsujo += sequestro;
          doc.crimeCooldown = Date.now();
          doc.save();
          let sequestroemb = embed(
        ...
        if (result === "Estupro") {
          let estupro = Math.floor(Math.random() * (20000 - 15000)) + 15000;
          doc.dinsujo += estupro;
          doc.crimeCooldown = Date.now();
          doc.save();
          let estuproemb = embed(
        ...
        if (result === "Manipulação") {
          let manipulacao = Math.floor(Math.random() * (1000 - 0)) + 1;
        ...
        if (result === "Hackear") {
          let hacker = Math.floor(Math.random() * (50000 - 30000)) + 30000;
          doc.dinsujo += hacker;
          doc.crimeCooldown = Date.now();
          doc.save();
          let embed = embed(
    ...
    if (!doc) {
      ...
      let semdoc = new discord.MessageEmbed()
        .setAuthor({ name: `» Tente Novamente.`, iconURL: client.err })
        .setColor(client.cor);
      return interaction.reply({ embeds: [semdoc] });
    }
  },
};
